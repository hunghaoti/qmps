...
for N in tqdm(ps):
    ...
    for _ in tqdm(T):
        ...
        res = minimize(time_evolve_measure_cost_fun, params, (A_[0], WW), 
		method = 'COBYLA', 
		options={'disp':True, 'tol':1.0e-10, 'catol':1.0e-3})
...
for q, i in enumerate(ps):
    ...
    np.save('qiskit_cobyla_10_3_lles', -np.log(np.array(lles)).T[0][:, j])
